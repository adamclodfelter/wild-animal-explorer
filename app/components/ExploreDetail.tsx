...
function Gallery({ query, color, name }: { query: string; color: string; name: string }) {
  ...
  return (
    <div style={{ height: "300px", background: "#0d0d1a", position: "relative", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      {images.length === 0 && !errored && <div style={{ fontSize: "13px", color: "rgba(255,255,255,0.25)", fontWeight: 700 }}>Loading photos...</div>}
...
export default function ExploreDetail({ item, backHref, backLabel, exploreMoreLabel, verse, verseRef }: Props) {
  if (!item) {
    return (
      <main style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Nunito', Arial, sans-serif", color: "white" }}>
        ...
        <Link href={backHref} style={{ marginTop: "20px", background: "#6C5CE7", color: "white", borderRadius: "50px", padding: "14px 32px", textDecoration: "none", fontWeight: 800 }}>{backLabel}</Link>
      </main>
    );
  }
  ...
  return (
    <main style={{ minHeight: "100vh", padding: "40px 20px", fontFamily: "'Nunito', Arial, sans-serif" }}>
      <div style={{ maxWidth: "640px", margin: "0 auto" }}>
        <Link href={backHref} style={{ display: "inline-block", background: "rgba(255,255,255,0.1)", borderRadius: "50px", padding: "12px 28px", fontWeight: 800, fontSize: "16px", color: "white", textDecoration: "none", marginBottom: "24px", border: "1px solid rgba(255,255,255,0.2)" }}>{backLabel}</Link>
        ...
        <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: "28px", overflow: "hidden", border: `2px solid ${item.color}66`, marginBottom: "16px" }}>
          <Gallery query={item.imageQuery} color={item.color} name={item.name} />
        ...
        <div style={{ background: `linear-gradient(135deg, ${item.color}18, ${item.color}08)`, borderRadius: "20px", padding: "20px 24px", marginBottom: "24px", border: `1px solid ${item.color}33` }}>
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: "14px" }}>
            <p style={{ fontSize: "14px", color: "rgba(255,255,255,0.5)", fontWeight: 600, lineHeight: 1.7, margin: "0 0 6px", fontStyle: "italic" }}>"{verse}"</p>
            <div style={{ fontSize: "12px", color: item.color, fontWeight: 800 }}>— {verseRef}</div>
          </div>
        </div>

        <Link href={backHref} style={{ display: "block", background: `linear-gradient(135deg, ${item.color}, ${item.color}99)`, color: "white", borderRadius: "50px", padding: "20px", textAlign: "center", textDecoration: "none", fontWeight: 900, fontSize: "20px" }}>{exploreMoreLabel}</Link>
